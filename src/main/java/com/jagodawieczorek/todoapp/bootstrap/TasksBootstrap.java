...
import static com.jagodawieczorek.todoapp.task.Status.READY;
...
public class TasksBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private void saveTasks(List<Category> categories) {
        ...
        for (int i = 1; i <= tasksCount; i++) {
            tasks.add(new Task(Integer.toString(i), "Test name", "Test description", categories.get(2), READY, null, null, null,  new Date(System.currentTimeMillis()), null,  new Date(System.currentTimeMillis())));
        }
